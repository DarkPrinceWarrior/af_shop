...
export function AuthPanel({ hideGuest = false }: AuthPanelProps) {
  const { user, isAuthenticated, login, register, logout } = useAuth();
  const { t } = useShop();
  const [mode, setMode] = useState<Mode>(hideGuest ? 'signIn' : 'guest');
  ...
  const tabs: Mode[] = hideGuest ? ['signIn', 'signUp'] : ['guest', 'signIn', 'signUp'];

  return (
    <div className="rounded-xl border border-border bg-card p-4">
      <div
        className={cn(
          'mb-4 grid gap-1 rounded-full bg-[var(--button-neutral-bg)] p-1',
          hideGuest ? 'grid-cols-2' : 'grid-cols-3',
        )}
      >
        {tabs.map((m) => (
          <button
