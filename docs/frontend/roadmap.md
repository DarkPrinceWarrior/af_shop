...
| 9 | Account & admin dashboards | ✅ done | `af_shop_front`; `/account/*` + `/admin/*` с RouteGuards, sidebar nav, CRUD UI |
| 10 | Production deployment plan | ⚪ later | TBD |
...
**Артефакты**: скриншоты складываются в `docs/frontend/screenshots/`. Коммитятся (по правилу «commit screenshots instead of full reports»).
...
- Скриншот: `docs/frontend/screenshots/01_catalog_redesign.png`.
...
**Backend контракт** (см. `docs/backend/api_frontend.md`):
...
- Скриншот: `docs/frontend/screenshots/05_checkout_auth_panel.png`.

---

## №9. Account & admin dashboards ✅

**Цель**: дать обычному покупателю личный кабинет, а суперпользователю —
админ‑панель с управлением заказами, каталогом, доставкой и пользователями.

**Backend контракт** уже доступен (см. `docs/backend/api_frontend.md`):

- Account: `GET /catalog/orders/me`, `GET /catalog/orders/me/{id}`,
  `PATCH /users/me`, `PATCH /users/me/password`
- Admin: `GET /admin/dashboard`, `GET /admin/orders` + filters,
  `PATCH/POST /admin/orders/{id}/{status,comment,cancel,complete}`,
  `GET/POST/PATCH/DELETE /admin/{categories,products,delivery-places}`,
  `POST /admin/media/images`, `GET/POST/PATCH/DELETE /users/`.

**Файлы**:

- `src/components/RouteGuards.tsx` — `RequireAuth`, `RequireSuperuser`.
  RequireAuth и RequireSuperuser редиректят на `/login` с
  `state.from = pathname`; RequireSuperuser дополнительно показывает
  access‑denied панель, если залогиненный юзер не суперюзер.
- `src/pages/LoginPage.tsx` — отдельная страница `/login` с центрированным
  `AuthPanel hideGuest`. После успешного логина — `navigate(from || '/account')`.
- `AuthPanel` — новый prop `hideGuest`: рендерит 2‑tab pill (Sign in / Sign up)
  без Guest, для использования вне checkout.
- TopBar: pill **Sign in** (`LogIn` icon) когда не авторизован → `/login`;
  pill **Overview** (`User`) когда авторизован → `/account`;
  pill **Dashboard** (`Shield`) когда суперюзер → `/admin`.
- `src/components/features/admin/{SidebarNav, OrderStatusBadge}.tsx` —
  переиспользуемые UI‑примитивы.
- `src/layouts/{AccountLayout, AdminLayout}.tsx` — wrapper с боковым меню +
  guard, использует `col-span-full` чтобы переписать catalog grid.
- `src/pages/account/{AccountHome, AccountOrders, AccountOrderDetail,
  AccountProfile}.tsx`
- `src/pages/admin/{AdminDashboard, AdminOrders, AdminOrderDetail,
  AdminProducts, AdminCategories, AdminDeliveryPlaces, AdminUsers}.tsx`
- CRUD‑формы — Sheet с inline‑полями (без отдельного диалога подтверждения,
  delete через `window.confirm`).
- `src/components/ui/select.tsx` использован для status filter / category select.
- API client расширен на ~25 методов для admin/user CRUD + `uploadMedia`.
- TopBar получил два новых пилла: `Overview` (когда залогинен) и
  `Dashboard` (когда `is_superuser`); линк "My orders" удалён, точка входа —
  `/account`.
- Старый `/orders/me` редиректит на `/account/orders`.
- i18n: `account.*`, `profile.*`, `admin.*`, `common.all` × en/ps/zh‑CN.

**e2e**:
- `e2e/admin.spec.ts` — admin sign in → /admin → orders / products / categories;
  normal user → /admin → access denied UI.
- `e2e/login-page.spec.ts` — TopBar Sign in pill ведёт на /login →
  admin login редиректит на /account; защищённый `/account/profile` без auth
  редиректит на /login и возвращает обратно после логина.

**Verify**:
- `npm run lint / typecheck / build / test:run / test:e2e` — все зелёные;
  12 e2e сценариев на chromium.
- Скриншоты: `07_account_home.png`, `08_admin_dashboard.png`,
  `09_admin_orders.png`, `10_login_page.png`.
...
- Скриншоты — в `docs/frontend/screenshots/`, имена с префиксом этапа: `02_*`, `04_*`.
